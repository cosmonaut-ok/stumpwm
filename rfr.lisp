...
(defvar *desktop-dump-file* (get-config-path "desktop.dump"))

(defvar *window-placement-dump-file* (get-config-path "window-placement.dump"))

(defvar *rfr-permanent-p* t)

(defvar *desktop-rules* nil)

(defvar *rfr-elements*
  '("all" "group" "screen" "window" "group-windows" "screen-windows" "all-windows" "desktop"))

(define-stumpwm-type :rfr (input prompt)
...
(defun update-group-in-rules (&optional (group (current-group)) (screen (current-screen)))
  ...
  (let* ((dumped-screen (dump-screen screen))
    ...
    (cond ((and
	  ...
	  (t
	   (push dumped-screen (ddump-screens *desktop-rules*)) ;; TODO: STUMPWM::*DESKTOP-RULES* ;note: deleting unreachable code
...
(defun recall-desktop (&key permanent-p)
  (progn
    (if (not (null permanent-p))
		 (if (probe-file *desktop-dump-file*)
		     (setf *desktop-rules*
			   (read-dump-from-file *desktop-dump-file*))))
...
(defun recall-window-placement (&key (window (current-window)) (permanent-p nil))
  (progn
    (if (not (null permanent-p))
		 (if (probe-file *window-placement-dump-file*)
		     (setf *window-placement-rules*
			   (read-dump-from-file *window-placement-dump-file*))))
    (sync-single-window-placement window)))
...
(defun recall-all ()
  ...
  (progn
    ...
    t) ;; unfortunately sync-single-window-placement gives nil, because dolist
  )
